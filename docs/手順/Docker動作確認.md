...
- `ROS_DISTRO` 等は自動で設定済み（`.zshrc`/`.bashrc` でsource済み）。手動のsourceは不要
- 対話シェルの既定は `zsh`（予測入力・補完メニュー・starshipプロンプト付き。詳細は `docker/README.md` 参照）
...
```bash
cd ~/Arbeit/docker && docker compose exec ros2 zsh
...
| 入る | `docker compose exec ros2 zsh` |
...
| ビルド結果の反映 | `source ~/ros2_ws/install/setup.bash`(zshなら`.zsh`)（新しいシェルなら自動） |
...
- 2026-07-17: dev/sim/driver 3コンテナの対話シェルの既定を`bash`から`zsh`に変更（予測入力・補完メニュー・starshipプロンプト等をホストの`~/.zshrc`を参考に軽量に再現）。上記コマンド例は本更新以降のもの。詳細は`docker/common/README.md`参照
