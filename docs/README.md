...
| [`推奨PCスペック.md`](推奨PCスペック.md) | 開発PCの推奨スペック(CPU/GPU/RAM)。負荷実測(#44)に基づく購入・選定の目安 |
...
- [`解説/経路生成のしくみ.md`](解説/経路生成のしくみ.md) — 直線プランナ→コストマップ+グリッド探索(NavFn)が何をしているかのやさしい解説
- [`解説/経路追従のしくみ.md`](解説/経路追従のしくみ.md) — DWB局所プランナ+cmd_vel安全機構が何をしているかのやさしい解説
...
- [`手順/実機Go2接続チェックリスト.md`](手順/実機Go2接続チェックリスト.md) — 実機Go2との有線LAN接続当日の手順書、および接続前に検証済みの事項(issue #3)
- [`手順/自己位置推定頑健性評価手順.md`](手順/自己位置推定頑健性評価手順.md) — 初期ズレ・対称環境・遮蔽・マルチパス・接触時オドメトリオーバーシュートの評価シナリオ・手順(issue #14)
