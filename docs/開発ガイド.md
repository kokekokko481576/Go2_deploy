...
| 自己位置推定(`go2_localization`) | ✓ M1/M2完了、M3進行中 | EKF(自M1)+AMCL(自M2)実装、テレオペでの動作確認・TF配信も確認済み(Issue #26のAMCLズレ修正込み)。頑健性評価(自M3、Issue #14)は着手済みで、GATE1計測中に**対称環境での誤収束・セッション内ドリフト蓄積(Issue #54)**を実例で確認済み。**ドリフト蓄積の対策・M1完了条件のドリフト率実測(Issue #10)はこれから** |
| 経路追従M2(`go2_path_following`) | ✓ BT導入+局所回避まで | Nav2 controller_server(DWB)+`nav2_bt_navigator`(Issue #50)で、Gazebo上のゴール到達・局所回避+リカバリ(Issue #23)まで確認済み。GATE1定量計測(到達成功率等)も実施したが、真値比較で**推定ベースの成功率に偽陽性が多数含まれることが判明**(Issue #36/#54)。**MPPI比較・偽陽性の原因修正はこれから** |
...
- `ros2_ws/src/go2_path_following`（M2: `nav2_controller`のDWBLocalPlannerを起動。`local_costmap`に
  `obstacle_layer`+リカバリ(追M3、Issue #23)を追加済み）。経路生成→追従→リカバリの判断は
  `nav2_bt_navigator`(Issue #50)のBT XML(`go2_bt_plugins`)に一本化され、旧`plan_follower`
  (手書きの状態機械)は削除済み。自作ノードは`goal_pose_bridge`(橋渡し)・`collision_detector`
  (接触検知)の2つだけになった。upstream本家のTF(フェーズA)・自作AMCL/EKFのTF
  (フェーズB、現在の既定)の両方でGazebo上のGo2が実際にゴールまで到達することを確認済み
  (Issue #21)
- GATE1の定量計測(到達成功率等)も実施した(Issue #36)が、Gazebo真値ツール(Issue #43)との
  比較で**推定ベースの成功率(46%)のうち大半が真値では未到達の偽陽性(真値ベースでは6%)**
  だったことが判明。原因は接触時の脚オドメトリオーバーシュート(Issue #14/#23)と、
  試行間でリセットしないセッション運用による**自己位置推定のドリフト蓄積**(Issue #54)
...
1. **セッション内ドリフト蓄積の修正(Issue #54)**: GATE1再計測で判明した、試行を重ねるほど
   自己位置推定の誤差が伸びる問題。これを解消しないとGATE1の到達成功率が「本当の性能」を
   反映しない(現在着手中)
2. **MPPIとの比較・選定**(Issue #22): DWB/MPPIを同時ロード済みだが、BTは
   `controller_id="FollowPath"`でDWBだけを使っている。`docker要件定義.md`・計画書どおり
   omni対応・障害物コストの扱いで比較し、どちらを本採用にするか決めるのはまだ
3. M4(最終アプローチ)・M5(跨ぎスキル切り替え)はまだ先。M4はアーム班とのゴール姿勢誤差の
   数値合意が前提(計画書3.8参照)

### 手を動かす場所

`go2_path_following/README.md`の「使い方(BT導入後の既定)」の手順がそのまま動作確認の
...
Issue #21)。定量ベースライン化(到達成功率・部材正対精度・ゴール姿勢誤差の実測)自体は
Issue #36で実施済みだが、真値比較(Issue #43)で**成功率の大半が偽陽性(セッション内ドリフト
蓄積、Issue #54)と判明**したため、GATE1はまだオープンのまま。#54の対策後に再計測し、
真値ベースの成功率が推定ベースと大きく食い違わなくなった時点でクローズ判断する。
...
## 8. 次の一歩チェックリスト(2026-08-05時点)

経路追従を仕上げる(§5)を優先する場合:

- [x] GATE1の定量ベースライン化(Issue #36): 実施済み。ただし真値比較で偽陽性多数と判明、
      根本原因は#54で追跡中
- [x] 局所回避+リカバリ(Issue #23、追M3): `local_costmap`に`obstacle_layer`+リカバリBTを実装済み
- [ ] **セッション内ドリフト蓄積の修正(Issue #54、現在着手中)**: 試行間リセット無しだと
      自己位置推定の誤差が伸びる問題。周期的再位置推定・AMCL/EKFパラメータ見直し等の対策を検討
- [ ] MPPIとの比較(Issue #22): `go2_path_following/config/`にMPPI設定は追加済みで
      `controller_id`で切り替え可能だが、数値比較そのものはまだ

並行して進められるもの:

- [ ] 自己位置推定M3(頑健性評価、Issue #14): 着手済み(対称環境での誤収束・接触時オドメトリ
      オーバーシュートを実例で確認済み)。評価手順の文書化は完了、残りシナリオの実測はこれから
...
- 2026-08-05: BT導入(#50)・局所回避+リカバリ(#23)・GATE1定量計測(#36)・セッション内ドリフト
  蓄積の発見(#54)を反映し、§1の状態表・§5・§6・チェックリストを更新(削除済みの旧
  `plan_follower`の記述を除き、現行のBT構成に合わせた)
