...
- go2_ros2_sim_py がiGPUで実用速度か未検証(段階1で最初に確認。ダメなら次点の go2_nav_simulation をDocker化する方針に切替)。
  2026-07-12時点: sim/driverコンテナ自体は未着手。今回の開発PC(Ubuntu 22.04, iGPU/AMD)では
  Ignition Gazebo(Fortress)単体のヘッドレス起動とRViz2のOpenGL 4.6アクセラレーションは確認できたため、
  iGPUでの土台は動く見込み。go2モデルを載せた実速度計測はsimコンテナ着手時に持ち越し
- Isaac Sim側との統合テスト環境をどちらのリポジトリが持つか(担当者と要調整)

## 7. 進捗（段階導入 §5 対応）

- §5-1「sim コンテナ単体で Gazebo + Nav2」: **未着手**。現状の `docker/` は §4.3 の dev コンテナ相当
  (`ros:humble` + Nav2 + robot_localization + slam_toolbox + pointcloud_to_laserscan + grid_map + ros_gz を
  1イメージに同居させた薄い開発環境)のみで、sim(go2_ros2_sim_py取り込み)・driver(unitree_ros2)は別イメージとして未作成
...
- Ignition Gazebo(Fortress)のコマンドは `gz` ではなく `ign gazebo` である点に注意(Humble向けros_gzの仕様)
