...
本体はいじらず、こちら側のcompose overrideで差分を持つのが基本方針:
...
例外: 顎3D LiDARの追加(2026-07-12)はxacro/launch本体の編集が必須で、compose overrideでは対応できない
ため、**forkして`external/go2_ros2_sim_py`のsubmodule参照先を自分のfork
(`kokekokko481576/go2_ros2_sim_py`)に切り替えた**(upstreamへのPR送付は意図しない。
upstream側は一切変更されず、forkは`upstream`リモートとして参照を残し追従可能)。詳細は§7参照。

同様のfork一次カスタマイズとして、`enable_nav2`起動引数を追加した(2026-07-17、Issue #35)。
launch本体の条件分岐が必要でcompose overrideでは対応できないため。既定`true`で従来挙動のまま、
`enable_nav2:=false`でupstream本家のNav2スタックを起動せず、dev側自作パイプラインの
GATE1定量計測時の`/robot1/cmd_vel`二重publish(計測汚染)を防ぐ。詳細は`docker/sim/README.md`参照。
...
- Phase 0 C1(ROS2環境構築)は Ubuntu 実機で動作確認済み(2026-07-12、詳細は `手順/Docker動作確認.md` §5)。
...
- 顎3D LiDARの追加(2026-07-12): `external/go2_ros2_sim_py` を自分のfork
  (`kokekokko481576/go2_ros2_sim_py`、public)に切り替え、`chin_lidar_frame` リンク+
  `gpu_lidar`センサ(垂直スキャン付き)を追加、`ros_gz_bridge`に`PointCloud2`のブリッジ行を追加した。
  ヘッドレスGazeboでのセンサロード(既存のlaser/imu/cameraと同一パターンの警告のみ、エラー無し)と
  ros_gz_bridgeでのブリッジ生成(`/robot1/chin_lidar/scan/points` ⇔ `PointCloud2`)まで確認。
  **搭載位置(x=0.29, z=-0.06)・ピッチ角・垂直FOV(16ch, ±15°)は実機スペックの根拠が無い仮値**
  (未使用の在庫URDFのHead_lowerを流用)。実際に点群データが流れる様子の目視確認(RViz2)は
  GUI環境(実デスクトップでの`docker compose up -d`)側で別途行うこと。詳細は
