...
詳細版は `docs/手順/Windows-WSL2セットアップ.md`。最短は次の3本を上から実行するだけ：
...
cd ~/Go2_deploy
# sim
cd docker/sim && docker compose build sim && docker compose up -d
# dev（別ターミナル）
cd ~/Go2_deploy/docker && docker compose build && docker compose up -d
docker compose exec ros2 zsh
```
GUIを使うならホストで一度 `xhost +local:docker`。
...
- **作業ブランチ**: `feat/<issue>-<topic>` 等。main へは rebase マージでリニア履歴を保つ。
