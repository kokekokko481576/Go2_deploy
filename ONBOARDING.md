...
実機DDS通信・GUIを素直に扱えるネイティブLinuxが本命。**Windows機なら
`docs/手順/デュアルブート構築.md` で Ubuntu 26.04 をデュアルブート導入するのが推奨。**

> **26.04 でもOK**（コンテナ中身は Ubuntu22.04 固定なので ROS2 はホストのUbuntu版に非依存）。
> **前提**: ストレージ空き **30〜40GB 推奨**、**RAM 16GB以上 推奨**。
...
### Windows（WSL2）（手軽に試す用・フォールバック）

Windows のまま手軽に試すなら WSL2 でも動く。ただし**実機Go2のDDS通信（#31）やGUI（#29）で
ネットワーク層の不確実性が出やすい**ため、本格運用・実機接続は上のデュアルブートを推奨。
詳細版・トラブルシュートは `docs/手順/Windows-WSL2セットアップ.md`。

> **WSL2ネイティブ運用**（WSL2内のUbuntuにDocker Engineを直接入れる。Docker Desktopは使わない）。
> **前提**: ストレージ空き **30〜40GB 推奨**（最低25GB）、**RAM 16GB以上 推奨**（`.wslconfig`でWSL2に12GB割当）。
...
> 詰まったら詰まり方を issue に記録（GUI=#29 / DDS疎通=#30 / 実機Go2=#31）。
