...
- 自分のEKF・AMCLは`publish_tf`/`tf_broadcast`を`true`にしてtfを配信するが、
  出力先をupstream本家の`/robot1/tf`とは別の**`/go2_localization/tf`**にlaunchファイルで
  remapし、名前空間を分離する(静止TF=`/tf_static`は引き続きupstreamの
  robot_state_publisherから読む)
- 推定結果(`amcl_pose`等)は`/go2_localization/...`という自分専用のトピックとしてのみ出力する
...
```yaml
      ...
      world_frame: odom
```
- `publish_tf: true`: `odom→base_link`のtfを配信する設定。出力先は下記`remappings`で
  upstreamと衝突しない専用トピック(`/go2_localization/tf`)にremapしている(なぜこの設計かの節参照)
...
- `remappings`: ノードが内部で使っているトピック名を、実際のトピック名に付け替える設定。
  ...
  - `/tf` → `/go2_localization/tf`: 自分のEKFが配信する`odom→base_link`の出力先。
    upstream本家の`/robot1/tf`とは別名にすることで衝突を避けている
  - `/tf_static` → `/robot1/tf_static`: 静止TF(センサ位置等)は自分では配信しないため、
...
amcl:
  ros__parameters:
    ...
    tf_broadcast: true
```
- `tf_broadcast: true`: EKFの`publish_tf`と同じ理由。AMCLは`map`→`odom`のtfを配信するが、
  下記`remappings`でupstreamと衝突しない専用トピック(`/go2_localization/tf`)にremapしている。
  推定結果自体は`amcl_pose`トピックでも別途受け取れる
...
- `odom_frame_id`: AMCLは内部で「`odom`→`base_link`のtfがどれだけ動いたか」を
  自分の動きモデルの入力として使う。下記`remappings`で`/tf`を`/go2_localization/tf`に
  remapしているため、**この`odom`→`base_link`のtfは自分のEKF(上記1節)が配信している
  ものを読みに行く**(2026-07-14以前はtf非配信でupstream側のtfを間借りしていたが、
  現在は自分のEKF・AMCLだけで完結したTFツリーになっている)
...
```
- `/tf` → `/go2_localization/tf`: `odom→base_link`の入力(自分のEKFの出力)も
  `map→odom`の出力(AMCL自身)も、この同じ専用トピックに乗せている
