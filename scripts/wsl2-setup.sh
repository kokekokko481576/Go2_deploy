...
if [ -e /dev/dri/renderD128 ] || [ -e /dev/dri/card0 ]; then
    ...
    warn "/dev/dri なし → docker compose up が失敗する可能性あり。GPUドライバ更新+wsl --shutdownで直らなければ docker/compose.override.yaml.example を参照(docs/手順/Windows-WSL2セットアップ.md のトラブルシュートに詳細)"
fi
